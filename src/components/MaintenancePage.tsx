  ...
  return (
    <div className="flex-1 p-6 overflow-y-auto min-w-0 w-full">
      <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">Maintenance AI</h2>
          <p className="text-sm text-slate-500">Dataset capteurs, prediction panne, rapports et demandes maintenance</p>
        </div>
        <button
          onClick={fetchOverview}
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white cursor-pointer border border-cyan-400/30 bg-cyan-500/15 hover:bg-cyan-500/25 transition-all"
        >
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={item.label} className="rounded-xl border border-white/[0.08] bg-slate-800/50 p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wide text-slate-500">AI</span>
            </div>
      ...
      {loading ? (
        <div className="text-center text-slate-500 py-12">Chargement maintenance AI...</div>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-4 mb-6">
            {(overview?.machines || []).map(machine => {
              ...
              return (
                <div key={`${machine.machineId}-${machine.node}`} className="rounded-xl border border-white/[0.08] bg-slate-800/50 p-4">
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div>
                      <div className="flex items-center gap-2">
                        <Bot size={16} className="text-cyan-300" />
                        <span className="text-sm font-bold text-white">{machine.machineName || machine.machineId}</span>
                      </div>
                      <div className="text-xs text-slate-500 mt-1">Node: {machine.node}</div>
                    </div>
                  ...
                  <div className="mb-3">
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-slate-400">Score anomalie</span>
                  ...
                  <div className="rounded-lg border border-white/[0.06] bg-slate-900/40 p-3 mb-3">
                    <div className="text-xs text-slate-500 mb-1">Prediction</div>
                    <div className="text-sm font-semibold text-white">{machine.prediction?.label || 'Pas de prediction'}</div>
                    <div className="text-xs text-slate-400 mt-1">ETA: {machine.prediction?.eta || '-'}</div>
                  </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-xl border border-white/[0.08] bg-slate-800/50 p-4">
              <div className="flex items-center gap-2 mb-4">
                <AlertTriangle size={16} className="text-amber-400" />
                <span className="text-sm font-bold text-white">Derniers rapports</span>
              </div>
              {(overview?.reports || []).length === 0 ? (
                <div className="text-center text-slate-500 py-8">Aucun rapport maintenance</div>
              ) : (
                    ...
                    return (
                      <div key={report._id} className="rounded-lg border p-3" style={{ background: st.bg, borderColor: st.border }}>
                        <div className="flex items-center justify-between gap-2 mb-1">
                          ...
                          <span className="text-[10px] text-slate-500">{new Date(report.createdAt).toLocaleString('fr-FR')}</span>
                        </div>
                        <div className="text-sm text-white font-medium">{report.prediction?.label || report.severity}</div>
                        <div className="text-xs text-slate-400 mt-1">{report.recommendedAction}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="rounded-xl border border-white/[0.08] bg-slate-800/50 p-4">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm font-bold text-white">Demandes maintenance</span>
              </div>
              {(overview?.requests || []).length === 0 ? (
                <div className="text-center text-slate-500 py-8">Aucune demande maintenance</div>
              ) : (
                <div className="flex flex-col gap-3">
                  {(overview?.requests || []).map(request => (
                    <div key={request._id} className="rounded-lg border border-white/[0.06] bg-slate-900/40 p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="text-sm font-bold text-white">{request.title}</div>
                          <div className="text-xs text-slate-500 mt-1">{request.machineName || request.machineId} - {new Date(request.createdAt).toLocaleString('fr-FR')}</div>
                        </div>
                        <span className="rounded-full bg-slate-700 px-2 py-0.5 text-[10px] font-bold text-slate-200">{requestStatusLabel(request.status)}</span>
                      </div>
