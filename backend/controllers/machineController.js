...
const buildMachinePayload = (req) => ({
  ...(req.body || {}),
  ...(req.file ? { imageUrl: `/uploads/machines/${req.file.filename}` } : {}),
});
...
const createMachine = async (req, res) => {
  try {
    const machine = await machineService.createMachine(buildMachinePayload(req));
...
const patchMachine = async (req, res) => {
  try {
    const machine = await machineService.updateMachine(req.params.id, buildMachinePayload(req));
    if (!machine) {
